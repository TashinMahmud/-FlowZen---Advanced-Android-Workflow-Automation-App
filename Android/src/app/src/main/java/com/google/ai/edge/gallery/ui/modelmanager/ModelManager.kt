...
fun ModelManager(
  ...
  // Navigate up when there are no models left (except for tasks that don't require models).
  LaunchedEffect(modelCount) {
    if (modelCount == 0 && task.type != com.google.ai.edge.gallery.data.TaskType.MAPS) {
      navigateUp()
    }
  }
