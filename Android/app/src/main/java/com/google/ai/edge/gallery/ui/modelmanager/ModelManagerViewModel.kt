...
private const val MODEL_ALLOWLIST_URL =
  "https://raw.githubusercontent.com/gummybear29/json-files/refs/heads/main/model_allowlist_3rd_aug_12.json"
private const val MODEL_ALLOWLIST_FILENAME = "model_allowlist_2nd.json"
...
open class ModelManagerViewModel(
  ...
  fun initializeModel(context: Context, task: Task, model: Model, force: Boolean = false) {
    viewModelScope.launch(Dispatchers.Default) {
      ...
      when (task.type) {
        ...
        TaskType.CREATE_WORKFLOW,
        TaskType.LLM_PROMPT_LAB,
        TaskType.CAMFLOW,
        TaskType.GMAIL_FORWARD ->
          LlmChatModelHelper.initialize(context = context, model = model, onDone = onDone)
        TaskType.TEST_TASK_1 -> { /* Optional: Handle or leave empty */ }
        TaskType.TEST_TASK_2 -> { /* Optional: Handle or leave empty */ }

        else -> {
          Log.w(TAG, "Unhandled task type: ${task.type}")
          onDone("Task type '${task.type}' not supported for initialization.")
        }
      }
    }
  }
  fun cleanupModel(task: Task, model: Model) {
    if (model.instance != null) {
      ...
      when (task.type) {
        ...
        TaskType.LLM_ASK_AUDIO,
        TaskType.CAMFLOW,
        TaskType.GMAIL_FORWARD -> LlmChatModelHelper.cleanUp(model = model)
  ...
  fun addImportedLlmModel(info: ImportedModel) {
    ...
    for (task in
    listOf(TASK_LLM_ASK_IMAGE, TASK_AI_ASSISTANT, TASK_LLM_ASK_AUDIO, TASK_LLM_PROMPT_LAB, TASK_LLM_CHAT,TASK_CREATE_WORKFLOW)) {
  ...
  fun loadModelAllowlist() {
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        TASK_AI_ASSISTANT.models.clear()
        TASK_LLM_ASK_AUDIO.models.clear()
        TASK_CREATE_WORKFLOW.models.clear()
        TASK_CAMFLOW.models.clear()
        for (allowedModel in modelAllowlist.models) {
          ...
          // Fixed: Only assign models to AI Assistant if explicitly specified in taskTypes
          if (allowedModel.taskTypes.contains(TASK_AI_ASSISTANT.type.id)) {
            TASK_AI_ASSISTANT.models.add(model)
            Log.d(TAG, "Added model ${model.name} to AI Assistant")
          }
          if (allowedModel.taskTypes.contains(TASK_LLM_ASK_AUDIO.type.id)) {
            TASK_LLM_ASK_AUDIO.models.add(model)
          }
          if (allowedModel.taskTypes.contains(TASK_CREATE_WORKFLOW.type.id)) {
            TASK_CREATE_WORKFLOW.models.add(model)
          }
          // Fixed: Only assign models to CamFlow if explicitly specified in taskTypes
          if (allowedModel.taskTypes.contains(TASK_CAMFLOW.type.id)) {
            TASK_CAMFLOW.models.add(model)
          }
        }
